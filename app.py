...
# Define the original CNN class (matches state_dict keys)
class CNN(nn.Module):
...
# Function to filter rare classes
...
def load_model(model_path, n_ch, n_times, n_classes):
    ...
    if n_classes != 10:  # Handle class mismatch
...
def display_evaluation_metrics(avg_test_loss, all_labels, all_preds):
    ...
    fig_metrics = go.Figure(data=[
        go.Bar(
            ...
            textposition="auto",
            marker_color=['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd']  # Distinct colors
        )
    ])
...
n_times = st.number_input("Number of time points", min_value=1, value=256, step=1)
n_classes = st.number_input("Number of classes", min_value=2, value=3, step=1)  # Set to 3 based on previous error
...
if cleaned_eeg_file is not None and labels_file is not None:
    ...
    # Resize EEG data to match n_times=256
